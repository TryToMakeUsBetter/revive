"""Web 对话客户端 —— 实现 ChatClient 接口，让 ChatBot 不改一行就能跑。

Web 是请求/响应模式：
- 用户 POST 一条消息 → 构造 IncomingMessage → 同步调 ChatBot.handle()
- handle() 内部调 client.send_text(reply, sender_id) —— 我们把 reply 暂存到当前线程
...
from .base import ChatClient, IncomingMessage, MessageHandler
